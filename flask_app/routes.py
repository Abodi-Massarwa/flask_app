...
def process_data(algorithm):
    ...
    if algorithm == 'algorithm1':
            ...
            result = per_category_round_robin(item_categories=item_categories, agent_category_capacities=category_capacities, initial_agent_order=initial_agent_order, alloc=AllocationBuilder(Instance(item_capacities=item_capacities, valuations=item_valuations)))
            ...
            result = capped_round_robin(item_categories=item_categories, agent_category_capacities=category_capacities, initial_agent_order=initial_agent_order, alloc=AllocationBuilder(Instance(item_capacities=item_capacities, valuations=item_valuations)),target_category=target_category)
            return render_template('result.html', result=result)
    elif algorithm == 'algorithm3':
            item_categories = ast.literal_eval(form['item_categories'])
            item_capacities = ast.literal_eval(form['item_capacities'])
            ...
            result = two_categories_capped_round_robin(item_categories=item_categories, agent_category_capacities=category_capacities, initial_agent_order=initial_agent_order, alloc=AllocationBuilder(Instance(item_capacities=item_capacities, valuations=item_valuations)),target_category_pair=target_category_pair)
            return render_template('result.html', result=result)
    elif algorithm == 'algorithm4':
            item_categories = ast.literal_eval(form['item_categories'])
            item_capacities = ast.literal_eval(form['item_capacities'])
            ...
            item_valuations = ast.literal_eval(form['item_valuations'])
            initial_agent_order = ast.literal_eval(form['initial_agent_order'])
            result = per_category_capped_round_robin(item_categories=item_categories, agent_category_capacities=category_capacities, initial_agent_order=initial_agent_order, alloc=AllocationBuilder(Instance(item_capacities=item_capacities, valuations=item_valuations)))
            return render_template('result.html', result=result)
    elif algorithm == 'algorithm5':
            item_categories = ast.literal_eval(form['item_categories'])
            item_capacities = ast.literal_eval(form['item_capacities'])
            category_capacities = ast.literal_eval(form['category_capacities'])
            item_valuations = ast.literal_eval(form['item_valuations'])
            result = iterated_priority_matching(item_categories=item_categories, agent_category_capacities=category_capacities, alloc=AllocationBuilder(Instance(item_capacities=item_capacities, valuations=item_valuations)))
            return render_template('result.html', result=result)
